...
def import_xes(filename: str):
    ...
    # Step 5: Extract only the date part (year-month-day)
    log['date_only'] = log['converted_timestamp'].dt.date

    # Step 6: Handle rows that failed to convert
    failed_rows = log[log['converted_timestamp'].isna()]

    if not failed_rows.empty:
        # Save failed rows with their original timestamps
        failed_rows[['original_timestamp', 'converted_timestamp']].to_csv("/Users/yachi/Desktop/FAIL_traffic_timestamp_conversion.csv", index=False)
        print(f"Failed conversions saved to 'FAIL_traffic_timestamp_conversion.csv' for review.")
    else:
        print("No failed conversions.")

    # Step 7: Save the original and successfully converted timestamps, along with the date_only column
    log.dropna(subset=['converted_timestamp'], inplace=True)  # Drop rows with NaT
    log[['original_timestamp', 'converted_timestamp', 'date_only']].to_csv("/Users/yachi/Desktop/traffic_finalconversion.csv", index=False)
...
def sort_log(df, case_id='case:concept:name', timestamp='date_only'):
...
def create_prefix_nextevent_pairs(df, max_prefix_length=19, case_id='case:concept:name', activity_id='concept:name'):
    X, y = [], []
...
filename = '/Users/yachi/Desktop/Road_Traffic_Fine_Management_Process.xes'
...
sorted_log.to_csv("/Users/yachi/Desktop/traffic_log_file.csv")
...
# Save to Excel
output_path = '/Users/yachi/Desktop/TRAFFIC__excel!.xlsx'
